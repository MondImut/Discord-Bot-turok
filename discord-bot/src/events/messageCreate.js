 ...
 *
 * Routes messages to: BoomBox (Turok v1.5), Scanner, Ticket threads,
 * Auto Thread, and the `!hesu` status command.
 *
 * BoomBox is now handled by Turok's MessageHandler (guild-aware, multi-provider,
 * SQLite-backed). The old Fandli BoomBox handler is retired.
 */
...
import { getBoomBoxMessageHandler }  from '../features/boombox/index.js';
...
export async function handleMessageCreate(message, secrets) {
  try {
    ...
    // Auto Thread — create a thread on non-thread channels that have it enabled
    ...
    // BoomBox v1.5.0 (Turok) — handles its own guild/channel/platform routing internally
    const boomboxHandler = getBoomBoxMessageHandler();
    if (boomboxHandler) {
      await boomboxHandler.handle(message);
    }
